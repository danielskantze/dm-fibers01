...
class DomainMapping {
  ...
  fromBToA(v: Vec3, clamp: boolean = false): Vec3 {
    const r = vec3.add(
      this.aMin,
      vec3.mul(vec3.div(vec3.sub(v, this.bMin), this.bDist), this.aDist)
    );
  ...
  fromAToB(v: Vec3, clamp: boolean = false): Vec3 {
    const r = vec3.add(
      this.bMin,
      vec3.mul(vec3.div(vec3.sub(v, this.aMin), this.aDist), this.bDist)
    );
...
export function createVec3({
  ...
  const componentRangeInputs = control.querySelectorAll(
    '.list-control input[type="range"]'
  ) as NodeListOf<HTMLInputElement>;
  const componentNumberInputs = control.querySelectorAll(
    '.list-control input[type="number"]'
  ) as NodeListOf<HTMLInputElement>;
  ...
  const inputH = control.querySelector(
    ".controls > input.horizontal"
  ) as HTMLInputElement;
  ...
  const { canvas, update: updateGimbal } = createVec3GimbalView(
    internalWidth,
    internalHeight
  );
