...
export function readTextureData(gl: WebGL2RenderingContext, texture: Texture): Uint8Array {
...
export function readOutputData(gl: WebGL2RenderingContext, output: StageOutput): Uint8Array {
  var framebuffer = output.framebuffer.framebuffer;
  const { width, height } = output.framebuffer;
  gl.bindFramebuffer(gl.FRAMEBUFFER, framebuffer);
  gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, output.textures[0].texture, 0);
  const data = new Uint8Array(width * height * 4);
  gl.readPixels(0, 0, width, height, gl.RGBA, gl.UNSIGNED_BYTE, data);
  return data;
}
