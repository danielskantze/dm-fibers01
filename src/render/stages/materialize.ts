...
function createTargetTextures(
  ...
  const startTimeTexture = createTexture(
    gl,
    width,
    height,
    "R32I",
  ...
  const targetTextures = [mainTexture, startTimeTexture];
...
function draw(
  ...
  gl.uniform2i(
    u.particlesTextureSize.location,
    input.targets[0].width,
    input.targets[0].height
  );
